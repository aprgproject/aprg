...
TEST(ReversingALinkedListTest, ThreeNodes) {
    ...
    node2->nextPointer = std::move(node3);
    node1->nextPointer = std::move(node2);

    std::unique_ptr<Node> result = reverseALinkedList(std::move(node1));

    EXPECT_EQ(3, result->value);
    result = std::move(result->nextPointer);
...
TEST(ReversingALinkedListTest, SevenNodes) {
    ...
    node6->nextPointer = std::move(node7);
    node5->nextPointer = std::move(node6);
    node4->nextPointer = std::move(node5);
    node3->nextPointer = std::move(node4);
    node2->nextPointer = std::move(node3);
    node1->nextPointer = std::move(node2);
