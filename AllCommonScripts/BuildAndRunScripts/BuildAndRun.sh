...
if [ "$scriptOption" == "clean" ]; then
    ...
    scriptPrint $scriptName $LINENO "The buildType is [$buildType]."
    scriptPrint $scriptName $LINENO "The cmakeGenerator is [$cmakeGenerator]."
	cmake -DCMAKE_BUILD_TYPE=$buildType -DCMAKE_C_COMPILER=/usr/bin/clang -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_C_FLAGS:STRING="${CMAKE_C_FLAGS} -g --coverage -fsanitize=address -fno-omit-frame-pointer" -DCMAKE_CXX_FLAGS:STRING="${CMAKE_C_FLAGS} -g --coverage -fsanitize=address -fno-omit-frame-pointer" "../$immediateDirectoryName/" "-G" "$cmakeGenerator"
