...
runStaticAnalyzersInDirectory() {
    ...
    "$buildAndRunScriptPath" build "StaticAnalyzersBuild" "Debug" | grep -E 'note:|style:|warning:|error:' | grep -Ev 'TestFactoryBase|test_info_|testing::|benchmark::|\/aprg\/benchmark\/benchmarkLibrary\/|\/aprg\/gtest\/gtest\/|\/aprg\/gsl\/gsl\/' | tee -a "$staticAnalysisFilename"
