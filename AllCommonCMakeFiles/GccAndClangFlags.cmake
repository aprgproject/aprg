...
set(APRG_COMPILER_FLAGS_FOR_DEBUG_WITH_DEBUG_INFO "-g --coverage")
...
set(APRG_LINKER_FLAGS_FOR_DEBUG "${APRG_COMPILER_FLAGS_FOR_RELEASE_NO_FLAGS}")
...
# -> Specifically:
# ---> it links to linalg.dll on Windows
# ---> it links to liblinalg.so (on Unix-like oses like Linux, BSD, AIX, …)
# ---> it links tolinalg.dylib on MacOSX.
...
# ---> It enables lots of flags as can be seen src (-ffloat-store, -ffast-math, -ffinite-math-only, -O3 …)
# -> -finline-functions
# -> -m64
# -> -funroll-loops
# -> -fvectorize
...
# ---> it may be better for embedded systems or anything where exceptions may not be acceptable.
# -> -fno-rtti:  Disable RTTI (Runtime Type Information)
# Fame and embedded systems developers report that they disable RTTI due to performance concerns.
...
# ->When all symbols are exported by default:
# ---> it may lead to undefined behavior if there a multiple definitions of same symbol during linking.
...
# ---> Its described in C++ Dialect Options and Objective-C and Objective-C++ Dialect Options.
...
# -fipa-reference-addressable, -fmerge-constants, -fmove-loop-invariants, -fmove-loop-stores,
...
#  -ftree-coalesce-vars, -ftree-copy-pro, -ftree-dc, -ftree-dominator-opts,
# -ftree-dse, -ftree-forwprop, -ftree-fre, -ftree-phiprop, -ftree-pta, -ftree-scev-cprop,
...
# -ftree-slp-vectorize, -funswitch-loops, -fvect-cost-model, -fvect-cost-model=dynamic,
...
# -> It turns on -ffast-math, -fallow-store-data-races and the Fortran-specific -fstack-arrays,
...
# -fprefetch-loop-arrays  -freorder-blocks-algorithm=st
...
# -> This option enables an static analysis of program flow
# --->  which looks for “interesting” interprocedural paths through the code, and issues warnings on them.
...
# ->  Enables all optimization that does not conflicts with debugging.
# ---> It can be used with the (-g) flag for enabling debugging symbols.
...
# -> When the object files are linked together, all the function bodies are instantiated
...
# -> It is recommended that you compile all the files participating in the same link
# ---> with the same options and also specify those options at link time.
# -> For example:
# ---> gcc -c -O2 -flto foo.c
# ---> gcc -c -O2 -flto bar.c
...
# -> Only variables that are actually allocated on the stack are considered,
# ---> optimized away variables or variables allocated in registers don’t count.

# Flag:  -fomit-frame-pointer
# -> This flag (-fno-omit-frame-pointer) is used for performance testing (particularly linux perf tool)
# --->  to save the information of the function call stack.
...
# ---> because the standard calling sequence automatically handles the frame pointer
...
# ---> and 32-bit Darwin x86 targets has been changed to -fomit-frame-pointer.
# ---> The default can be reverted to -fno-omit-frame-pointer by configuring GCC
...
# -> you get an error message from the linker indicating that -fpic does not work;
# ---> in that case, recompile with -fPIC instead.
# -> (These maximums are 8k on the SPARC and 28k on AArch64. The x86 has no such limit.)
...
# ---> but the generated position-independent code can be only linked into executables.
...
# ---> You can use this to supply system-specific linker options that GCC does not recognize.
...
# ---> to pass arguments to linker options using the option=value syntax than as separate arguments.
# ---> For example, you can specify -Xlinker -Map=output.map rather than -Xlinker -Map -Xlinker output.map.
...
# ------> It stores information that can be useful for debugging purporses,
# --------> for instance, to print out a stacktrace when a crash happens.
# ------> A case where it could make sense to remove the symbols table
# --------> would be a scenario where you have serious constraints of storage capacity
# ------> (in that regard, gcc -Os -s or make CXXFLAGS="-Os -s" ... is useful as
# --------> it will result in a smaller slower binary that is also stripped to reduce size further).
...
# ---> and prevents linking with the shared libraries.
...
# ---> should use the shared libgcc instead of the static version.
# The most common of these is when the application wishes to throw
# ---> and catch exceptions across different shared libraries.
...
# ---> you may find that they are not always linked with the shared libgcc.
...
# Otherwise, it takes advantage of the linker
...
# -> However, if a library or main executable is supposed to throw or catch exceptions,
# ---> you must link it using the G++ driver,
...
# -> Windows: *.exe
# -> *.hex -> Extension of many compiled firmwares generated
# ---> by embedded systems compilers such as proprietary compilers for Microcontrollers.
...
# NOTE: Sanitizers are not static analysis, there are dynamic analysis tools
# -> because they are using compiler instrumentation to run the program.

# Using AddressSanitizer
# In order to use AddressSanitizer you will need to COMPILE
# -> and LINK your program using clang with the -fsanitize=address switch.
...
# -> and can be enabled with ASAN_OPTIONS=detect_leaks=1 on x86_64 OS X.
# LSan lies dormant until the very end of the process,
# -> at which point there is an extra leak detection phase.
# In performance-critical scenarios, LSan can also be used without ASan instrumentation.
# Stand-alone mode
# If you just need leak detection, and don't want to bear the ASan slowdown,
# -> you can build with -fsanitize=leak instead of -fsanitize=address.
# This will link your program against a runtime library containing
# -> just the bare necessities required for LeakSanitizer to work.
...
# Make sure to use clang++ (not ld) as a linker,
# -> so that your executable is linked with proper UBSan runtime libraries.
# You can use clang instead of clang++ if you’re compiling/linking C code.
# You can enable only a subset of checks offered by UBSan,
...
# -> and in general need to be accompanied by a suitable -fsanitize= flag.
...
# In most cases you may want to combine libFuzzer with AddressSanitizer (ASAN),
# -> UndefinedBehaviorSanitizer (UBSAN), or both.
