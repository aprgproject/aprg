...
public:
    ...
    [[nodiscard]] virtual Key getLongestPrefixOf(Key const& keyToCheck) const = 0;  // get the longest key that has a prefix
    ...
    [[nodiscard]] virtual Strings getAllKeysWithPrefix(Key const& prefix) const = 0;         // all the keys having s as a prefix
    [[nodiscard]] virtual Strings getAllKeysThatMatch(Key const& patternToMatch) const = 0;  // all the keys having s as a prefix
};
