...
ValueUnion GetSysctlImp(std::string const& Name) {
    ...
    if (sysctlbyname(Name.c_str(), nullptr, &CurBuffSize, nullptr, 0) == -1) return ValueUnion();
...
std::vector<CPUInfo::CacheInfo> GetCacheSizesMacOSX() {
    ...
    struct {
        ...
        {"hw.l1dcachesize", "Data", 1, CacheCounts[1]},
...
double GetCPUCyclesPerSecond(CPUInfo::Scaling scaling) {
    ...
    if (IsWindowsXPOrGreater() && SUCCEEDED(SHGetValueA(
        ...
        return static_cast<double>(static_cast<int64_t>(data) * static_cast<int64_t>(1000 * 1000));  // was mhz
    }
#elif defined(BENCHMARK_OS_SOLARIS)
...
#if (                                                                                                 \
    defined BENCHMARK_OS_FREEBSD || defined(BENCHMARK_OS_LINUX) || defined BENCHMARK_OS_MACOSX ||     \
