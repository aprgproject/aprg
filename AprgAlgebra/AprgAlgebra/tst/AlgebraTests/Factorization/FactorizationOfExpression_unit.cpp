...
TEST(FactorizationOfExpressionTest, FactorizeAnExpressionWorksOnMultiplicationAndDivisionExpression) {
    Polynomial const polynomial({Monomial(1, {{"x", 2}}), Monomial(-4, {})});
    Expression const expressionToTest(createExpressionIfPossible({polynomial, "*", ln("x"), "/", sin("x")}));
    ...
    Term const termToExpect3(ln("x"));
    Term const termToExpect4(createExpressionIfPossible({sin("x"), "^", -1}));
...
TEST(FactorizationOfExpressionTest, FactorizeAnExpressionWorksOnAdditionAndSubtractionExpression) {
    ...
    Term const part1(createExpressionIfPossible({Monomial(12, {{"x", 5}}), "*", sin("x")}));
    Term const part2(createExpressionIfPossible({Monomial(4, {{"x", 7}}), "*", sin("x"), "*", cos("x")}));
    Term const part3(createExpressionIfPossible({Monomial(8, {{"x", 9}}), "*", sin("x"), "*", sin("x")}));
    Expression const expressionToTest(createExpressionIfPossible({part1, "+", part2, "-", part3}));

    Terms termsToVerify(factorizeAnExpression(expressionToTest));

    Term const termToExpect1(4);
    Term const termToExpect2(Monomial(1, {{"x", 5}}));
    Term const termToExpect3(createExpressionIfPossible(
        {3, "+", Monomial(1, {{"x", 2}}), "*", cos("x"), "-", Monomial(2, {{"x", 4}}), "*", sin("x")}));
    Term const termToExpect4(sin("x"));
    ASSERT_EQ(4U, termsToVerify.size());
    EXPECT_EQ(termToExpect1, termsToVerify[0]);
    EXPECT_EQ(termToExpect2, termsToVerify[1]);
    EXPECT_EQ(termToExpect3, termsToVerify[2]);
...
TEST(FactorizationOfExpressionTest, FactorizeAnExpressionWorksOnRaiseToPowerExpression) {
    ...
    Term const base(createExpressionIfPossible({sin("x"), "*", cos("x")}));
    Expression const expressionToTest(createExpressionIfPossible({base, "^", 17}));

    Terms termsToVerify(factorizeAnExpression(expressionToTest));

    Term const termToExpect1(createExpressionIfPossible({cos("x"), "^", 17}));
    Term const termToExpect2(createExpressionIfPossible({sin("x"), "^", 17}));
...
TEST(FactorizationOfExpressionTest, FactorizeAnExpressionWorksDerivativeDefinition) {
    ...
    Term const deltaX("deltaX");
    Term const deltaXPlusX(Polynomial{Monomial(1, {{"deltaX", 1}}), Monomial(1, {{"x", 1}})});
    Term const part1(Monomial(1, {{"deltaX", 1}, {"x", AlbaNumber::createFraction(1, 3)}}));
    Term const addend1(Monomial(1, {{"deltaX", 1}, {"x", AlbaNumber::createFraction(2, 3)}}));
    Term const addend2(createExpressionIfPossible({part1, "*", deltaXPlusX, "^", AlbaNumber::createFraction(1, 3)}));
    Term const addend3(createExpressionIfPossible({deltaX, "*", deltaXPlusX, "^", AlbaNumber::createFraction(2, 3)}));
    Expression const expressionToTest(createExpressionIfPossible({addend1, "+", addend2, "+", addend3}));

    Terms termsToVerify(factorizeAnExpression(expressionToTest));

    string const stringToExpect1("deltaX");
    string const stringToExpect2("(1[x^(2/3)]+(1[x^(1/3)]*((1[deltaX] + 1[x])^(1/3)))+((1[deltaX] + 1[x])^(2/3)))");
    ASSERT_EQ(2U, termsToVerify.size());
...
TEST(FactorizationOfExpressionTest, FactorizeAnExpressionWorksWhenSomeAddendsAreSame) {
    ...
    Term const cos2X(cos(twoX));
    Term const addend1(createExpressionIfPossible({3, "*", cos2X, "^", 2}));
    Term const addend2(createExpressionIfPossible({cos2X, "^", 3}));
    Expression const expressionToTest(
        createExpressionIfPossible({1, "+", cos2X, "+", cos2X, "+", cos2X, "+", addend1, "+", addend2}));

    Terms termsToVerify(factorizeAnExpression(expressionToTest));

    string const stringToExpect1("(1+cos(2[x])+cos(2[x])+cos(2[x])+(3*cos(2[x])*cos(2[x]))+(cos(2[x])^3))");
    ASSERT_EQ(1U, termsToVerify.size());
    EXPECT_EQ(stringToExpect1, convertToString(termsToVerify[0]));
}
