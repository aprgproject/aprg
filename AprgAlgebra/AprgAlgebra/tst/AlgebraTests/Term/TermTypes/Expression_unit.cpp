...
TEST(ExpressionTest, EqualityOperatorWorks) {
    ...
    Expression const expression2(
        createExpressionIfPossible({5, "+", createExpressionIfPossible({5, "+", "interest"})}));
    Expression const expression3(createExpressionIfPossible({6, "+", "interest"}));
    Expression const expression4(createExpressionIfPossible({5}));
    Expression const expression5(createExpressionIfPossible({5, "+", createExpressionIfPossible({5, "+"})}));
    Expression const expression6(
        createExpressionIfPossible({5, "+", createExpressionIfPossible({5, "+", "interest"})}));
...
TEST(ExpressionTest, LessThanOperatorWorks) {
    Expression const expression1;
    Expression const expression2(createExpressionIfPossible({6, "+", "interest"}));
    Expression const expression3(createExpressionIfPossible({6}));
    Expression const expression4(createExpressionIfPossible({6, "+", "interest", "+", "debt"}));
    Expression const expression5(createExpressionIfPossible({5, "+", "interest"}));
    Expression const expression6(createExpressionIfPossible({7, "+", "interest"}));
...
TEST(ExpressionTest, GetFirstTermWorks) {
    Expression const expression1(createExpressionIfPossible({5412}));
    Expression const expression2(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expression3(createExpressionIfPossible({"-", 16}));
...
TEST(ExpressionTest, GetCommonOperatorLevelWorks) {
    Expression const expression1;
    Expression const expression2(createExpressionIfPossible({5}));
    Expression const expression3(createExpressionIfPossible({6, "+", "interest"}));
    Expression const expression4(createExpressionIfPossible({6, "-", "interest"}));
    Expression const expression5(createExpressionIfPossible({6, "*", "interest"}));
    Expression const expression6(createExpressionIfPossible({6, "/", "interest"}));
    Expression const expression7(createExpressionIfPossible({6, "^", "interest"}));
...
TEST(ExpressionTest, ConstructionWorks) {
    ...
    TermWithDetails const termWithDetails2(Term(20), TermAssociationType::Negative);
    Expression const expression1;
    Expression const expression2(Term(12));
    Expression const expression3(OperatorLevel::AdditionAndSubtraction, {});
    Expression const expression4(OperatorLevel::AdditionAndSubtraction, {termWithDetails1, termWithDetails2});
...
TEST(ExpressionTest, GetTermsWithAssociationWorks) {
    Expression const expression(createExpressionIfPossible({695, "-", "interest", "+", "debt"}));
...
TEST(ExpressionTest, GetDebugStringWorks) {
    Expression const expression1;
    Expression const expression2(createExpressionIfPossible({695, "-", "interest", "+", "debt"}));
    Expression expression3;
    expression3.putTermWithDivisionIfNeeded(Term(96));
    Expression const expression4(createExpressionIfPossible({expression2, "^", "cash"}));
...
TEST(ExpressionTest, ContainsOnlyOneTermWorks) {
    Expression const expression1;
    Expression const expression2(createExpressionIfPossible({5}));
    Expression const expression3(createExpressionIfPossible({"-", 5}));
    Expression const expression4(createExpressionIfPossible({6, "+", "interest"}));
...
TEST(ExpressionTest, ExpressionThatIsConstructedWithTermsWithDetailsHasIsSimplifiedFlagNotSet) {
    TermWithDetails const termWithDetails1(Term(10), TermAssociationType::Positive);
    TermWithDetails const termWithDetails2(Term(20), TermAssociationType::Negative);
    Expression const expression(OperatorLevel::AdditionAndSubtraction, {termWithDetails1, termWithDetails2});
...
TEST(ExpressionTest, PutTermWithAdditionUsingNullExpressionWorks) {
    ...
    Expression const expressionToExpect1(createOrCopyExpressionFromATerm(0));
    Expression const expressionToExpect2(createOrCopyExpressionFromATerm(0));
    Expression const expressionToExpect3(createOrCopyExpressionFromATerm(0));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expressionToExpect5(createExpressionIfPossible({695, "*", "interest"}));
...
TEST(ExpressionTest, PutTermWithAdditionUsingConstantWorks) {
    ...
    Expression const subExpression(createExpressionIfPossible({695, "*", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({42}));
    Expression const expressionToExpect2(createExpressionIfPossible({42}));
    Expression const expressionToExpect3(createExpressionIfPossible({42}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "+", "interest", "+", 42}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression, "+", 42}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
    EXPECT_EQ(expressionToExpect4, expression4);
...
TEST(ExpressionTest, PutTermWithAdditionUsingExpressionWithSameOperationLevelWorks) {
    ...
    Expression const expressionToExpect4(createExpressionIfPossible({695, "+", "interest", "+", "x", "-", "y"}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression, "+", "x", "-", "y"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
    EXPECT_EQ(expressionToExpect4, expression4);
...
TEST(ExpressionTest, PutTermWithAdditionUsingExpressionWithDifferentOperationLevelWorks) {
    ...
    Expression const subExpression2(createExpressionIfPossible({695, "*", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({"x", "/", "y"}));
    Expression const expressionToExpect2(createExpressionIfPossible({"x", "/", "y"}));
    Expression const expressionToExpect3(createExpressionIfPossible({"x", "/", "y"}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "+", "interest", "+", subExpression1}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression2, "+", subExpression1}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
    EXPECT_EQ(expressionToExpect4, expression4);
...
TEST(ExpressionTest, PutTermWithSubtractionUsingExpressionWithSameOperationLevelWorks) {
    ...
    Expression const expressionToExpect3(expressionToExpect1);
    Expression const expressionToExpect4(createExpressionIfPossible({695, "+", "interest", "-", "x", "+", "y"}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression, "-", "x", "+", "y"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
...
TEST(ExpressionTest, PutTermWithMultiplicationUsingNullExpressionWorks) {
    ...
    Expression const expressionToExpect3(createExpressionIfPossible({"-", 1}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "*", "interest"}));
    Expression const expressionToExpect5(createExpressionIfPossible({695, "+", "interest"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
...
TEST(ExpressionTest, PutTermWithMultiplicationUsingConstantWorks) {
    ...
    Expression const expressionToExpect1(createExpressionIfPossible({42}));
    Expression const expressionToExpect2(createExpressionIfPossible({42}));
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "*", 42}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "*", "interest", "*", 42}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression, "*", 42}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
...
TEST(ExpressionTest, PutTermWithMultiplicationUsingExpressionWithSameOperationLevelWorks) {
    ...
    Expression const expressionToExpect1(createExpressionIfPossible({"x", "/", "y"}));
    Expression const& expressionToExpect2(expressionToExpect1);
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "*", expressionToExpect1}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "*", "interest", "*", "x", "/", "y"}));
    Expression const expressionToExpect5(
...
TEST(ExpressionTest, PutTermWithDivisionUsingExpressionWithSameOperationLevelWorks) {
    ...
    expressionToExpect1.reverseTheAssociationOfTheTerms();
    Expression const expressionToExpect2(expressionToExpect1);
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "/", expressionToApply}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "/", "interest", "/", "x", "*", "y"}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression, "/", "x", "*", "y"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
...
TEST(ExpressionTest, PutTermWithRaiseToPowerWorks) {
    ...
    Expression const expressionInExponent(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({42}));
    Expression const expressionToExpect2(createExpressionIfPossible({1, "^", 42}));
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "^", 42}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "^", "interest", "^", 42}));
    Expression const expressionToExpect5(createExpressionIfPossible({expressionInExponent, "^", 42}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
    EXPECT_EQ(expressionToExpect4, expression4);
...
TEST(ExpressionTest, PutTermWithRaiseToPowerUsingExpressionWithSameOperationLevelWorks) {
    ...
    Expression const subExpression1(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({"x", "^", "y"}));
    Expression const expressionToExpect2(
        createExpressionIfPossible({1, "^", createExpressionIfPossible({"x", "^", "y"})}));
    Expression const expressionToExpect3(
        createExpressionIfPossible({"(", "-", 1, ")", "^", createExpressionIfPossible({"x", "^", "y"})}));
    Expression const expressionToExpect4(
        createExpressionIfPossible({695, "^", "interest", "^", createExpressionIfPossible({"x", "^", "y"})}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression1, "^", "x", "^", "y"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
    EXPECT_EQ(expressionToExpect4, expression4);
...
TEST(ExpressionTest, PutTermWithRaiseToPowerUsingExpressionWithDifferentOperationLevelWorks) {
    ...
    Expression const subExpression2(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({"x", "-", "y"}));
    Expression const expressionToExpect2(createExpressionIfPossible({1, "^", subExpression1}));
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "^", subExpression1}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "^", "interest", "^", subExpression1}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression2, "^", subExpression1}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
...
TEST(ExpressionTest, PutTermWithRaiseToPowerUsingExpressionWithNegativeAssociationExpression) {
    ...
    Expression const expressionToApply(createExpressionIfPossible({"-", "5"}));
    expression1.putTermWithRaiseToPowerIfNeeded(Term(expressionToApply));
    expression2.putTermWithRaiseToPowerIfNeeded(Term(expressionToApply));
    expression3.putTermWithRaiseToPowerIfNeeded(Term(expressionToApply));
    expression4.putTermWithRaiseToPowerIfNeeded(Term(expressionToApply));
    ...
    Expression const subExpression1(createExpressionIfPossible({"-", "5"}));
    Expression const subExpression2(createExpressionIfPossible({695, "+", "interest"}));
    Expression const expressionToExpect1(createExpressionIfPossible({"-", "5"}));
    Expression const expressionToExpect2(createExpressionIfPossible({2, "^", subExpression1}));
    Expression const expressionToExpect3(createExpressionIfPossible({"(", "-", 1, ")", "^", subExpression1}));
    Expression const expressionToExpect4(createExpressionIfPossible({695, "^", "interest", "^", subExpression1}));
    Expression const expressionToExpect5(createExpressionIfPossible({subExpression2, "^", subExpression1}));
    EXPECT_EQ(expressionToExpect1, expression1);
...
TEST(ExpressionTest, PutPolynomialFirstWithMultiplicationWorks) {
    ...
    Expression const expressionToExpect1(createExpressionIfPossible(
        {createExpressionIfPossible({Monomial(1, {{"x", 1}})}), "+",
         createExpressionIfPossible({Monomial(1, {{"y", 1}})})}));
    Expression const expressionToExpect2(createExpressionIfPossible(
    ...
    Expression const expressionToExpect3(createExpressionIfPossible(
    ...
    Expression const expressionToExpect4(createExpressionIfPossible(
...
TEST(ExpressionTest, SimplifyWorksOnPutPolynomialFirstWithMultiplication) {
    ...
    Expression const expressionToExpect2(createOrCopyExpressionFromATerm(Polynomial{
        Monomial(1, {{"a", 1}, {"x", 1}}), Monomial(1, {{"a", 1}, {"y", 1}}), Monomial(-1, {{"b", 1}, {"x", 1}}),
        Monomial(-1, {{"b", 1}, {"y", 1}}), Monomial(100, {{"x", 1}}), Monomial(100, {{"y", 1}})}));
    Expression const expressionToExpect3(createOrCopyExpressionFromATerm(
...
TEST(ExpressionTest, PutExpressionWithMultiplicationWorksOnExpressionWithEmptyExpressionAndWillNotHaveImpact) {
    Term const term1(createExpressionIfPossible({2, "^", "x"}));
    Term const term2(Polynomial{Monomial(1, {{"x", 1}}), Monomial(1, {})});
    Term const term3(1);
    Expression expressionToTest;
    Expression const expressionToApply(createExpressionIfPossible({term1, "+", term2, "+", term3}));

    expressionToTest.putExpressionWithMultiplication(expressionToApply);

    string const stringToExpect1("((2^x)+(1[x] + 1)+1)");
...
TEST(ExpressionTest, PutExpressionWithMultiplicationWorksOnAdditionAndSubtractionExpression) {
    ...
    Expression const subExpression2(createExpressionIfPossible({"c", "+", "d"}));
    Expression expression1;
    ...
    Expression const expressionToApply1(createExpressionIfPossible({"x", "+", "y", "+", 1}));
    Expression const expressionToApply2(expression3);
    ...
    string const stringToExpect1("(x+y+1)");
    string const stringToExpect2("((a*x)+(a*y)+a+(b*x)+(b*y)+b-(c*x)-(c*y)-c-(d*x)-(d*y)-d)");
    string const stringToExpect3("(((a+b)/(c+d)*x)+((a+b)/(c+d)*y)+((a+b)/(c+d)))");
    string const stringToExpect4("((((a+b)^(c+d))*x)+(((a+b)^(c+d))*y)+((a+b)^(c+d)))");
    string const stringToExpect5("((a*(a+b)/(c+d))+(b*(a+b)/(c+d))-(c*(a+b)/(c+d))-(d*(a+b)/(c+d)))");
...
TEST(ExpressionTest, PutExpressionWithMultiplicationWorksOnMultiplicationAndDivisionExpression) {
    Expression const subExpression1(createExpressionIfPossible({"a", "*", "b"}));
    Expression const subExpression2(createExpressionIfPossible({"c", "*", "d"}));
    Expression const subExpression3(createExpressionIfPossible({"x", "*", "x"}));
    Expression const subExpression4(createExpressionIfPossible({"y", "*", "y"}));
    Expression expression1;
    Expression expression2(createExpressionIfPossible({subExpression1, "-", subExpression2}));
    Expression expression3(createExpressionIfPossible({subExpression1, "/", subExpression2}));
    Expression expression4(createExpressionIfPossible({subExpression1, "^", subExpression2}));
    Expression expression5(createExpressionIfPossible({subExpression1, "-", subExpression2}));
    Expression const expressionToApply1(createExpressionIfPossible({subExpression3, "-", subExpression4}));
    ...
    string const stringToExpect1("((x*x)-(y*y))");
    string const stringToExpect2("((a*b*x*x)-(a*b*y*y)-(c*d*x*x)+(c*d*y*y))");
    string const stringToExpect3("((a*b/c/d*x*x)-(a*b/c/d*y*y))");
    string const stringToExpect4("((((a*b)^(c*d))*x*x)-(((a*b)^(c*d))*y*y))");
    string const stringToExpect5("((a*b*x*x)-(c*d*x*x))");
    EXPECT_EQ(stringToExpect1, convertToString(expression1));
    EXPECT_EQ(stringToExpect2, convertToString(expression2));
    EXPECT_EQ(stringToExpect3, convertToString(expression3));
    EXPECT_EQ(stringToExpect4, convertToString(expression4));
...
TEST(ExpressionTest, SimplifyWorksOnPutExpressionWithMultiplication) {
    ...
    Expression const expressionToExpect1(createExpressionIfPossible({subExpression3, "-", subExpression4}));
    Expression const expressionToExpect2(createExpressionIfPossible(
    ...
    Expression const subExpressionToExpect1(createExpressionIfPossible({"c", "^", "d"}));
    Expression const subExpressionToExpect2(createExpressionIfPossible({"b", "*", subExpressionToExpect1}));
    Expression const subExpressionToExpect3(createExpressionIfPossible({"a", "^", subExpressionToExpect2}));
    Expression const expressionToExpect4(createExpressionIfPossible(
...
TEST(ExpressionTest, SetWorks) {
    ...
    Expression const expressionToExpect(createExpressionIfPossible({100, "-", 2}));
...
TEST(ExpressionTest, SetTermWorks) {
    ...
    Expression const expressionToExpect1(createExpressionIfPossible({Term()}));
    Expression const expressionToExpect2(createExpressionIfPossible({87}));
    Expression const expressionToExpect3(createExpressionIfPossible({78, "+", "energy"}));
    EXPECT_EQ(expressionToExpect1, expression1);
    EXPECT_EQ(expressionToExpect2, expression2);
    EXPECT_EQ(expressionToExpect3, expression3);
}
...
TEST(ExpressionTest, SimplifyWorksAsSkippedWhenIsSimplifiedIsSet) {
    ...
    Expression const expressionToExpect(createExpressionIfPossible({100, "+", 200, "-", 50}));
    EXPECT_EQ(expressionToExpect, expressionToTest);
}
