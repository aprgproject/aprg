...
typedef enum EAaSysComGwProtocol{
  ...
  EAaSysComGwProtocol_GTPf                  =  6  /**< GTP-f protocol - supported only in RCP MCU*/
} EAaSysComGwProtocol;
...
typedef enum EAaSysComGwTcpType
{
  ...
  EAaSysComGwTcpType_ServerFirstConnected   = 4    /**< TCP server with single client (first connected). */
} EAaSysComGwTcpType;
...
typedef enum EAaSysComGwRouteStatus
{
    ...
    EAaSysComGwRoute_LinkRejected       = 4     /**< Link is rejected. */
