...
if [ "$scriptRunningOption" == "outputWithHighlighting" ]; then
	...
	"$aprgDirectory/AllCommonScripts/WslScripts/BuildAndRunInWsl.sh" $2 $3
	exitCode=$?
	...
	"$aprgDirectory/AllCommonScripts/WslScripts/BuildAndRunInWsl.sh" $2 $3 2>&1 | sed -E "s|\/mnt\/(\w+)\/|\U\1:/|g"
	exitCode=$?
	...
	"$aprgDirectory/AllCommonScripts/WslScripts/BuildAndRunInWsl.sh" $2 $3 2>&1 | sed -E "s|$(pwd)||g"
	exitCode=$?
else
