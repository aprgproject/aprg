...
//
// -> COROLLARY. If a computer’s instruction set includes an instruction for each of the 16 Boolean functions of two
// variables,
// ---> then any Boolean function of three variables can be implemented with FOUR (or fewer) instructions.
// -> One instruction implements g(x, y), another implements h(x, y), and these are combined with "and" and "exclusive
// or".
//
// -> Example: Consider the Boolean function that is 1 if exactly two of x, y, and z are 1:
// ---> f(x, y, z) = xyz' + xy'z + x'yz
// -> Before proceeding, the interested reader might like to try to implement f with four instructions, without using
// the theorem.
